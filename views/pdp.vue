<template>
  <div class="all">
    <div>
      <h1>{{ this.msg_page_name }}</h1>
      <h2>{{ this.msg_intro_of_tool }}</h2>
    </div>

    <div v-if="!showDashboard" class="container">
    ...
    <!-- <div v-else class="container_dashboard">
      <h1>{{ dashboard_name }}</h1>

      <hr>

      <div class="container_dashboard_elements">
        <h2><strong>My Designs</strong></h2>
      </div>
...
// import DynamicTable from '@/components/result_table.vue';
...
export default {
  ...
  data () {
    ...
    return {
      ...
      dashboard_name: 'BOKE Panel Design Tool DashBoard',
  ...
  methods: {
    ...
    sendDataToBackend() {
      ...
      if (this.formData.inputPaste && this.formData.inputFile) {
        ...
        missingFields.push('Only one of Input Sequence and Input File should be provided, and we have clean the paste input. Place click the submit button again.');
...
hr {
    width: 95%;  /* 设置线的宽度 */
    border: none;  /* 去除默认边框 */
    border-top: 1px solid #ccc;  /* 添加顶部边框以显示线 */
    margin: 0 0 0 0;
}

.container_dashboard_elements {
    width: 90%;
    text-align: left;
}
</style>
